...
ML / Robot AI 논문을 최근 기간 기준으로 수집하고, 키워드와 규칙 기반 점수로 정렬한 뒤, CLI 또는 Streamlit GUI에서 빠르게 실험할 수 있는 로컬 하네스입니다.

현재 버전은 다음 흐름에 초점을 맞춥니다.

- arXiv에서 최근 논문 수집
- 선택적으로 Semantic Scholar 메타데이터 보강
...
- Streamlit GUI
  ...
  - `Fetch` 후에는 네트워크 재호출 없이 rerank 즉시 반영
  - preset 저장 / 불러오기
- CLI 실행
  - 설정 파일 기준으로 한 번에 fetch -> enrich -> rank -> export
- 코어 모듈 분리
...
```bash
streamlit run paper_radar_app.py
```
...
3. ranking 설정을 바꾸면 현재 fetch 결과에 대해 즉시 재정렬됩니다.
